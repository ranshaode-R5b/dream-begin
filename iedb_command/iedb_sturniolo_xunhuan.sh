...
tool_consensus="/home/renshida/apps/IEDB_3.1.12"
	result_consensus="$path_result/iedb_sturniolo"
	cd $tool_consensus ||{ echo "Failed to change directory to $tool_netmhccons"; exit 1; }
	com_file="$path_code/iedb_sturniolo.sh"
	for file_path in "$path_fasta"/*.fa; do
		...
		echo "python $tool_consensus/mhc_II_binding.py sturniolo $file_name $file_path $number > $result_consensus/$cleaned_name.txt" >> "$com_file"
	done
