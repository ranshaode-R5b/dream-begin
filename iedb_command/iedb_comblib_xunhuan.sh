...
tool_consensus="/home/renshida/apps/IEDB_3.1.12"
	...
	com_file="$path_code/iedb_comblib_command.sh"
	for file_path in "$path_fasta"/*.fa; do
		...
		echo "python $tool_consensus/mhc_II_binding.py comblib $file_name $file_path $number > $result_consensus/$cleaned_name.txt" >> "$com_file"
		done
